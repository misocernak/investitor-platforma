...
<div class="inline-flex items-center gap-1 p-1 rounded-lg bg-surface-container-low" role="group" aria-label="Prodajni status stana">
  ...
  <form method="POST" action="{{ route('units.status', $stan) }}" @if($vrednost === 'Prodat_u_procesu_uknjizenja' && !$aktivan) data-potvrdi="Označiti stan {{ $stan->oznaka }} kao prodat? Oglas se uklanja sa Temelja." @endif>
    ...
    <button type="submit" @disabled($aktivan) class="inline-flex items-center gap-1.5 h-8 px-3 rounded font-label-md text-label-md transition-colors {{ $aktivan ? ($vrednost === 'Prodat_u_procesu_uknjizenja' ? 'bg-emerald-600 text-white' : ($vrednost === 'Rezervisan' ? 'bg-amber-500 text-white' : 'bg-primary text-on-primary')) : 'text-on-surface-variant hover:bg-surface-container hover:text-on-surface' }}">
