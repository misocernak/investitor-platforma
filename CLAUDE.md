...
- Dokumenti za kupca: `documents.vidljivo_kupcu` (prekidač sa ikonicom oka). Kupac potvrđenog stana vidi aktivne verzije dokumenata svoje zgrade i svog stana. Temelj traži spisak (`/api/temelj/kupac/dokumenti`) i link (`/api/temelj/kupac/dokument-link`) → potpisan URL `temelj.dokument` važi 5 min i ponovo proverava pristup. Fajlovi se nikad ne kopiraju na Temelj.
- Reklamacije sa Temelja: kupac potvrđenog stana prijavljuje (`/api/temelj/kupac/reklamacija`, `claims.izvor = temelj`) i dopisuje se (`/kupac/reklamacija-poruka`, `claim_notes.od_kupca`). Kupac vidi samo reklamacije svog stana gde je on kupac i beleške sa `vidljivo_kupcu`. Promena statusa ili poruka kupcu → Temelj šalje kupcu mejl (`/api/v1/kupac/obavestenje`). Stranica "Moj stan" dobija sve jednim pozivom (`/api/temelj/kupac/pregled`).
